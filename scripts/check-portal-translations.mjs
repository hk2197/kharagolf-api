 ...
 * portal web bundle's i18n bundles (Task #2053).
 *
 * The mobile app already has an equivalent guard
 * (`scripts/check-mobile-translations.mjs`) that fails the build when a
 * non-English locale is missing a key or silently falls back to the
 * English source string. The portal web bundle had no such guard, so a
 * future contributor could land a new English-only key and ship without
 * anyone noticing until a non-English producer reported it.
 *
 * This script walks every bundle under
 *
 *   artifacts/kharagolf-web/src/i18n/locales/en/<file>.json
 *
 ...
 *   - scripts/portal-translations-baseline.json: a generated grandfather
 ...
 *   --update-baseline   rewrite scripts/portal-translations-baseline.json
...
const BASELINE_PATH = join(
  ...
  "portal-translations-baseline.json",
);
...
/**
 * Strict-translation prefixes.
 *
 * Some key namespaces are too important to ever fall back to English:
 * a missing or untranslated entry would silently re-introduce the
 * regression the originating task was meant to fix. The lint refuses
 * to baseline anything under these prefixes — even if
 ...
 * dotted-key prefix. Prefix match is left-anchored at a dot: an entry
 * with keyPrefix `"foo.bar."` matches any leaf under that subtree but
 * does NOT match `foo.barLegacy.*`.
 *
 * No portal-side strict prefixes are required at introduction time —
 * the mechanism is preserved so future regressions can be guarded the
 * same way as the mobile lint.
 */
export const STRICT_TRANSLATION_PREFIXES = [
  // Per-notification row descriptions on the notification preferences
  // page (Task #2165). The companion lint
  ...
  { file: "portal.json", keyPrefix: "notificationKeys." },
];
...
function buildBaselineDoc(failures) {
  ...
  const doc = {
    $comment:
      "Generated by `pnpm run lint:portal-translations:update-baseline`. " +
  ...
  for (const f of failures) {
    ...
    // Strict-translation keys are never grandfathered, so we also refuse
    // to write them into a freshly-rebuilt baseline. Otherwise
    // `--update-baseline` would silently hide a regression and the next
    // plain lint run would not catch it.
...
export function applyBaseline(failures, baseline) {
  ...
  const seen = new Set();
  for (const f of failures) {
    // Strict-translation keys bypass the baseline entirely — even if a
    // stale baseline still lists them, the failure is re-promoted to
    // active so the build catches the regression.
    if (
...
function runSelfTest() {
  const cases = [
    // ---- missing-key ----
    {
      ...
      args: { lang: "hi", file: "portal.json", key: "loyalty.empty", source: "Empty", value: undefined },
      expectKinds: ["missing-key"],
    },
    {
      name: "missing key in English locale is ignored",
      args: { lang: "en", file: "portal.json", key: "loyalty.empty", source: "Empty", value: undefined },
      expectKinds: [],
    },
    // ---- same-as-english ----
    {
      name: "byte-identical English copy fails",
      args: { lang: "de", file: "portal.json", key: "loyalty.section", source: "Loyalty & Rewards", value: "Loyalty & Rewards" },
      expectKinds: ["same-as-english"],
    },
    {
      name: "FIR loanword stays untranslated and passes",
      args: { lang: "de", file: "portal.json", key: "roundTable.fir", source: "FIR", value: "FIR" },
      expectKinds: [],
    },
    {
      name: "Bunker loanword stays untranslated and passes",
      args: { lang: "de", file: "portal.json", key: "caddieLie.bunker", source: "Bunker", value: "Bunker" },
      expectKinds: [],
    },
    {
      name: "Format loanword stays untranslated and passes",
      args: { lang: "de", file: "portal.json", key: "scoringHistory.format", source: "Format", value: "Format" },
      expectKinds: [],
    },
    {
    ...
    },
    {
      ...
      args: { lang: "hi", file: "portal.json", key: "myProfile", source: "My Profile", value: "मेरी प्रोफ़ाइल" },
      expectKinds: [],
    },
    {
    ...
    // ---- type-mismatch ----
    {
      ...
      args: { lang: "de", file: "portal.json", key: "statsLabels", source: "Stats", value: { tournaments: "Turniere" } },
    ...
    },
    {
      ...
      args: { lang: "ja", file: "portal.json", key: "newSectionTitle", source: "Brand New Section", value: undefined },
      expectKinds: ["missing-key"],
    },
  ];
  ...
  // Baseline behaviour.
  const fakeFailures = [
    { lang: "de", file: "portal.json", key: "old.key", kind: "same-as-english", detail: "..." },
    { lang: "de", file: "portal.json", key: "new.key", kind: "same-as-english", detail: "..." },
    { lang: "fr", file: "common.json", key: "missingOldOne", kind: "missing-key", detail: "..." },
    { lang: "de", file: "portal.json", key: "structuralBug", kind: "type-mismatch", detail: "..." },
  ];
  const baseline = new Set([
    "same-as-english::de::portal.json::old.key",
    "missing-key::fr::common.json::missingOldOne",
    "same-as-english::de::portal.json::alreadyFixed",
  ]);
  const part = applyBaseline(fakeFailures, baseline);
  const wantActive = [
    "same-as-english::de::portal.json::new.key",
    "type-mismatch::de::portal.json::structuralBug",
  ];
  ...
  // ----- strict-translation prefix mechanism -----
  // The portal lint ships with no strict prefixes today, so we exercise
  // the mechanism with a synthetic prefix injected for the duration of
  // the test. This guarantees the mechanism still catches a regression
  // even though no portal-side prefixes are wired up at introduction.
  STRICT_TRANSLATION_PREFIXES.push({
    file: "portal.json",
    keyPrefix: "selfTest.strict.",
  });
  try {
    if (
      !isStrictTranslationKey("portal.json", "selfTest.strict.someKey")
    ) {
      failed += 1;
      console.error(
        `  ✗ isStrictTranslationKey matches the synthetic test prefix in portal.json`,
      );
    } else {
      console.log(
        `  ✓ isStrictTranslationKey matches the synthetic test prefix in portal.json`,
    ...
    if (
      isStrictTranslationKey("common.json", "selfTest.strict.someKey")
    ) {
      failed += 1;
      console.error(
        `  ✗ isStrictTranslationKey is scoped to the matching file (portal.json only)`,
      );
    } else {
      console.log(
        `  ✓ isStrictTranslationKey is scoped to the matching file (portal.json only)`,
    ...
    if (
      isStrictTranslationKey("portal.json", "selfTest.strictLegacy.foo")
    ) {
      failed += 1;
      console.error(
        `  ✗ isStrictTranslationKey prefix match is left-anchored at a dot (rejects strictLegacy)`,
      );
    } else {
      console.log(
    ...
    const strictBaseline = new Set([
      "same-as-english::ja::portal.json::selfTest.strict.label",
      "same-as-english::ja::portal.json::someOther.legitimate.key",
    ]);
    const strictPart = applyBaseline(strictFailures, strictBaseline);
    if (
      !strictPart.active.some(
        (f) =>
          f.lang === "ja" &&
          f.key === "selfTest.strict.label",
      )
    ) {
      failed += 1;
      console.error(
    ...
    if (
      strictPart.active.some(
        (f) => f.key === "someOther.legitimate.key",
      )
    ) {
      failed += 1;
      console.error(
        `  ✗ applyBaseline still grandfathers non-strict failures alongside strict ones`,
      );
    } else {
      console.log(
        `  ✓ applyBaseline still grandfathers non-strict failures alongside strict ones`,
    ...
    const jaPortalBucket = fakeBaselineDoc.sameAsEnglish.ja?.["portal.json"] ?? [];
    if (jaPortalBucket.includes("selfTest.strict.label")) {
      failed += 1;
      console.error(
        `  ✗ buildBaselineDoc drops strict-translation keys (they must never be baselined)`,
      );
    } else {
      console.log(
        `  ✓ buildBaselineDoc drops strict-translation keys (they are never baselined)`,
      );
    }
    if (!jaPortalBucket.includes("someOther.legitimate.key")) {
      failed += 1;
      console.error(
    ...
    STRICT_TRANSLATION_PREFIXES.pop();
  }

  const totalCases = cases.length + 11;
  if (failed > 0) {
...
function main() {
  ...
  try {
    ...
    console.error(`check-portal-translations: ${err.message}`);
    process.exit(2);
  }
  ...
  if (updateBaseline) {
    ...
    console.log(
      `check-portal-translations: baseline written to ${relative(repoRoot, BASELINE_PATH)} (${missingCount} missing keys + ${sameCount} same-as-English entries grandfathered).`,
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      `check-portal-translations: failed to read baseline at ${relative(repoRoot, BASELINE_PATH)}: ${err.message}`,
    );
    process.exit(2);
  }
  ...
  if (active.length > 0) {
    ...
    console.error(
      `\ncheck-portal-translations: found ${active.length} new issue${active.length === 1 ? "" : "s"} across ${grouped.size} locale bundle${grouped.size === 1 ? "" : "s"}:\n`,
    );
    for (const bucket of buckets) {
      ...
      const summary = Object.entries(counts)
        ...
        .join(", ");
      console.error(
        `  artifacts/kharagolf-web/src/i18n/locales/${bucket}  (${summary})`,
      );
    ...
    console.error(
      "\nFix the translations above, or — if a value is a legitimate loanword / brand / autonym — extend ALLOWED_IDENTICAL_VALUES in scripts/check-portal-translations.mjs.",
    );
    console.error(
      `${grandfathered.length} pre-existing issue${grandfathered.length === 1 ? "" : "s"} are grandfathered via ${relative(repoRoot, BASELINE_PATH)}. Translate them and run \`pnpm run lint:portal-translations:update-baseline\` to shrink the baseline.\n`,
    );
    process.exit(1);
  }
  ...
  if (staleBaselineEntries.length > 0) {
    console.log(
      `check-portal-translations: ${staleBaselineEntries.length} baseline entr${staleBaselineEntries.length === 1 ? "y is" : "ies are"} no longer needed. Run \`pnpm run lint:portal-translations:update-baseline\` to clean up.`,
    );
  }

  console.log(
    `check-portal-translations: scanned ${scannedKeys} key checks across ${scannedLocales} locale bundles (${enFiles.length} English source files, ${allLangs.length - 1} non-English locales) — no NEW missing keys or untranslated fallbacks. ${grandfathered.length} pre-existing issue${grandfathered.length === 1 ? "" : "s"} grandfathered via ${relative(repoRoot, BASELINE_PATH)}.`,
  );
}
