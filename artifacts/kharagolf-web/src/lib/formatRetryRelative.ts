 ...
 * by every notify-retry badge surface (wallet withdrawal, side-game
 * settlement receipt, wallet top-up refund). The mobile mirror lives at
 * `artifacts/kharagolf-mobile/lib/formatRetryRelative.ts` so the two
 * platforms can never silently diverge — both are unit-tested with the
