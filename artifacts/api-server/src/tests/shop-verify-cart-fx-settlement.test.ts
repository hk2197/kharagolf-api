/**
 * Regression test for Task #1573 — concurrent verify-cart calls for the
 * same Stripe Checkout session must NOT record two FX ledger rows.
 *
 * Same race-condition family as Task #1307 (which gated the receipt email
 * + Razorpay push behind `flippedShopOrders.length > 0`). The FX
 * settlement insert in `verify-cart` was previously unconditional, so two
 * verify-cart requests landing concurrently for the same session would
 * both reach `recordCheckoutSettlement` with the same `pendingOrders`
 * snapshot and write two `fx_ledger_entries` rows for one payment —
 * inflating the org's settled-amount totals and making FX reconciliation
 * noisier.
 *
 * The fix mirrors the existing receipt + push gating: the FX settlement
 * block now only runs for the request that actually flipped the order(s)
 * to paid.
 *
 * Setup notes:
...
async function setupOrgWithUsdProfile(opts: { sessionId: string }) {
  ...
  const [org] = await db.insert(organizationsTable).values({
    name: `Shop FX Settlement Test ${suffix}`,
    slug: `shop-fx-settle-${suffix}`,
  ...
  const [user] = await db.insert(appUsersTable).values({
    replitUserId: `shop-fx-settle-${suffix}`,
    username: `shop_fx_settle_${suffix}`,
    role: "player",
...
describe("POST /shop/orders/verify-cart → FX settlement is recorded once per cart (Task #1573)", () => {
  it("only records one FX ledger row when verify-cart is posted concurrently for the same Stripe session", async () => {
    const sessionId = `cs_test_${Date.now()}_${Math.random().toString(36).slice(2, 7)}_fxrace`;
    ...
    // win the status-flip UPDATE — the other must NOT write a second FX
    // ledger row.
    ...
    expect(ledgerRows[0]!.sourceType).toBe("shop_cart");
  ...
  it("records exactly one FX ledger row on a normal (non-concurrent) settlement", async () => {
    const sessionId = `cs_test_${Date.now()}_${Math.random().toString(36).slice(2, 7)}_fxsingle`;
    const { org, user, order } = await setupOrgWithUsdProfile({ sessionId });
    ...
    // duplicate FX ledger row — the gating check runs before the insert.
