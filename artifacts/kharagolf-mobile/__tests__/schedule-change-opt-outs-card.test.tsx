/**
 * UI tests: mobile `ScheduleChangeOptOutsCard` (Task #1688 — mobile mirror of
 * the web ScheduleChangeOptOutsCard from Task #387 / Task #512).
 *
 * Verifies:
 *   1. The card self-hides on 401/403 (non-admin user).
 *   2. The card renders the empty-state copy when the GET returns [].
 *   3. The card lists opted-out rows with name, email, and opted-out date.
 *   4. Tapping "Re-subscribe" DELETEs the opt-out and removes the row.
 *   5. Switching from an unauthorized org to an authorized one re-shows it.
 */
...
const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  const deleteMatch = url.match(/\/organizations\/(\d+)\/bounced-digest-schedule-opt-outs\/(\d+)$/);
  if (deleteMatch && method === "DELETE") {
    deletedUserIds.push(parseInt(deleteMatch[2], 10));
    return new Response(null, { status: 204 });
  }
  if (url.endsWith("/bounced-digest-schedule-opt-outs") && method === "GET") {
    if (getStatus === 200) {
...
describe("ScheduleChangeOptOutsCard (Task #1688)", () => {
  it("self-hides when the API returns 403 (non-admin user)", async () => {
    getStatus = 403;
    const { container } = render(
      <ScheduleChangeOptOutsCard orgId={7} token="t" />,
    );
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalled();
    });
    await waitFor(() => {
      expect(container.querySelector('[data-testid="card-schedule-change-opt-outs"]')).toBeNull();
    });
  });
  ...
  it("renders nothing when orgId or token is missing (no API call)", async () => {
    const { container } = render(
      <ScheduleChangeOptOutsCard orgId={null} token={null} />,
    );
    expect(container.querySelector('[data-testid="card-schedule-change-opt-outs"]')).toBeNull();
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("renders the empty state when no one has opted out", async () => {
    getBody = [];
    render(<ScheduleChangeOptOutsCard orgId={7} token="t" />);
    expect(
      await screen.findByTestId("text-no-schedule-opt-outs"),
    ).toBeInTheDocument();
  });

  it("lists each opted-out member with name, email, and date", async () => {
    getBody = [
      {
        userId: 11,
    ...
    expect(screen.getByTestId("button-resubscribe-schedule-11")).toBeInTheDocument();
    expect(screen.getByTestId("button-resubscribe-schedule-22")).toBeInTheDocument();
  });
  ...
  it("re-shows the card after switching from an unauthorized org to an authorized one", async () => {
    ...
    rerender(<ScheduleChangeOptOutsCard orgId={9} token="t" />);

    expect(await screen.findByText("Carol Director")).toBeInTheDocument();
    expect(
      container.querySelector('[data-testid="card-schedule-change-opt-outs"]'),
    ).not.toBeNull();
  });

  it("re-subscribing a member DELETEs the opt-out and removes the row", async () => {
    getBody = [
      {
        userId: 11,
        email: "alice@example.com",
    ...
    render(<ScheduleChangeOptOutsCard orgId={7} token="t" />);
    const btn = await screen.findByTestId("button-resubscribe-schedule-11");
    ...
    expect(screen.getByTestId("text-no-schedule-opt-outs")).toBeInTheDocument();
  });
});
